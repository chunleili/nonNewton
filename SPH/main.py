...
class SPHBase:
    ...
    def compute_boundary_volume_task(self, p_i, p_j, delta: ti.template()):
        if meta.ps.material[p_j] == meta.ps.material_solid:
        ...
        for i in range(1):
            for r_obj_id in meta.ps.object_id_rigid_body:
                if meta.ps.object_collection[r_obj_id]["isDynamic"]:
                    ...
                    self.enforce_boundary_3D(meta.ps.material_solid)
    ...
    def step(self):
        ...
        if meta.ps.dim == 2:
            self.enforce_boundary_2D(meta.ps.material_fluid)
        elif meta.ps.dim == 3:
            self.enforce_boundary_3D(meta.ps.material_fluid)
...
class DFSPHSolver(SPHBase):
    ...
    def compute_densities_task(self, p_i, p_j, ret: ti.template()):
        x_i = meta.ps.x[p_i]
        if meta.ps.material[p_j] == meta.ps.material_fluid:
            # Fluid neighbors
            x_j = meta.ps.x[p_j]
            ret += meta.ps.m_V[p_j] * self.cubic_kernel((x_i - x_j).norm())
        elif meta.ps.material[p_j] == meta.ps.material_solid:
            # Boundary neighbors
            ## Akinci2012
            x_j = meta.ps.x[p_j]
            ret += meta.ps.m_V[p_j] * self.cubic_kernel((x_i - x_j).norm())
    ...
    def compute_densities(self):
        # for p_i in range(meta.ps.particle_num[None]):
        for p_i in ti.grouped(meta.ps.x):
            if meta.ps.material[p_i] != meta.ps.material_fluid:
                continue
    ...
    def compute_density_error(self, offset: float) -> float:
        ...
        for I in ti.grouped(meta.ps.x):
            if meta.ps.material[I] == meta.ps.material_fluid:
    ...
    def predict_velocity(self):
        ...
        for p_i in ti.grouped(meta.ps.x):
            if meta.ps.is_dynamic[p_i] and meta.ps.material[p_i] == meta.ps.material_fluid:
                meta.ps.v[p_i] += self.dt[None] * meta.ps.acceleration[p_i]
